...
public class MspInstaller
{
	...
	{
		String query = "CREATE TABLE `audit` ( `id` INT NOT NULL AUTO_INCREMENT, `userId` INT NOT NULL, `action` VARCHAR(45) NOT NULL, `result` VARCHAR(45) NOT NULL, PRIMARY KEY (`id`), INDEX `userId_idx` (`userId` ASC), CONSTRAINT `userId`   FOREIGN KEY (`userId`)   REFERENCES `lm`.`users` (`id`)   ON DELETE NO ACTION   ON UPDATE NO ACTION);";
		if ( st != null )
		{
			st.executeUpdate( query );
		}
